...
document
  .getElementsByClassName("selectInput")[0]
  .addEventListener("change", () => (delay = changeSpeed()));
document.getElementById("done").addEventListener("click", () => done());
document
  .getElementsByClassName("btn1")[0]
  .addEventListener("click", () => generatebars());
document
...
function resizeWidth() {
  ...
  for (let i = 0; i < bars.length; i++) {
    ...
    bars[i].style.transform = `translateX(${
      i * (width / 2 / bars.length + 2)
    }px)`;
  }
}
...
function generatebars(array = []) {
  ...
  document.querySelector(".sortedArray").innerHTML =
    "sorted array will display here after Visualization";
  for (let i = 0; i < array.length; i++) {
    ...
    bar.style.transform = `translateX(${
      i * (window.innerWidth / 2 / array.length + 2)
    }px)`;
...
document.getElementById("mode").addEventListener("click", (e) => {
  ...
  if (app[0] === "light") {
    ...
    document
      ...
      .setAttribute(
        "src",
        "https://cdn-icons-png.flaticon.com/128/12301/12301305.png"
      );
  } else {
    app.add("light");
    document
      ...
      .setAttribute(
        "src",
        "https://img.icons8.com/?size=80&id=H3yHeysB1dxv&format=png"
      );
  }
});
